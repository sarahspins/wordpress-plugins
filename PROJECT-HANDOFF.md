...
GitHub release: `suite-16`

| Component | Published version |
| --- | ---: |
| Dash Connector | 1.7.21 |
| Programming | 1.9.2.4 |
| Productions | 3.7.1 |
| Rink Displays | 2.8.10 |
| Productions Theme | 1.6.7 |

Suite 16 contains all five ZIPs and `ice-field-updates.json`. It publishes the tested Dash Connector Event Assignment Rules, resilient read-only API requests, event-type correction tools, and string-safe event-type dropdowns, together with the tested Rink Displays responsive timing and background schedule-refresh improvements. The GitHub wiki update is pending explicit approval.

## Current state

- Dash Connector 1.7.21 adds administrator-managed Event Assignment Rules, bounded read-only retries, and an explicit bulk Event Type Correction mode with DaySmart event-type dropdowns. Event Type IDs remain strings so letter IDs are preserved through search, updates, verification, history, and Undo.
...
- Rink Displays 2.8.10 includes the external schedule loader, scheduled media, adaptive Later pagination, stable pinned regions, locker/registration enrichment, a wider responsive time column with a 4px fitting floor, reduced row padding, atomic static schedule JSON, refresh monitoring, and a three-minute TV heartbeat that queues deduplicated background schedule rebuilds.
...
- Observe Rink Displays 2.8.10 on the shows website and LG screens, especially responsive timing and background registration-total refreshes.
